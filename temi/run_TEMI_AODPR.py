...
from temi.models import ConvAttenFusion, MTAEncoder, MTADecoder, Aligner
import torch.optim
...
def train(predict_model, embed_encoder, embed_decoder, aligner, train_iter, test_iter, CLASSES, CE_loss, SIM_loss, REC_loss, optimizer, scheduler, args, last_epoch):
    ...
    for epoch in range(num_epochs):
        ...
        for X, UMZ, M, y, MZ, pid, _ in train_iter:
            ...
            loss = ce_loss + rec_loss + ect_loss + sim_loss 
        ...
        logger.info("Epoch[%d/%d], loss:%.4f, CE loss:%.4f, SIM loss:%.4f, REC loss: %.4f, ECON loss: %.4f" 
                    % (epoch+1, num_epochs, loss_ / batch_count, ce_loss_ / batch_count, sim_loss_ / batch_count, 
                       rec_loss_ / batch_count, ect_loss_ / batch_count))
        ...
        writer.add_scalars("LOSS", {"ALL": loss_ / batch_count, "CE": ce_loss_ / batch_count, 
                                    ...
                                    "ECON": ect_loss_ / batch_count}, epoch + 1)
        ...
        if (epoch+1) == num_epochs and args.savedisk:
            ...
            torch.save(state2, "{}/embed_decoder.pt".format(args.model_path))
            ...
            torch.save(state3, "{}/aligner.pt".format(args.model_path))
...
def main(args, CLASSES, model, model_architecture, time_tab):
    ...
    train(predict_model, embed_encoder, embed_decoder, aligner, train_iter, test_iter, CLASSES, 
          CE_loss, SIM_loss, REC_loss, optimizer, scheduler, train_args, args.last_epoch)
...
def cli_main():
    ...
    embed_encoder = MTAEncoder(model_architecture)
    embed_decoder = MTADecoder(model_architecture)


    predict_model_zero = ConvAttenFusion(model_architecture, class_num)
    embed_encoder_zero = MTAEncoder(model_architecture)
    embed_decoder_zero = MTADecoder(model_architecture)
